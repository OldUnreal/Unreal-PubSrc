...
final function bool IsUnrealTournamentClient()
{
	return (NetConnection(Player) && GetNegotiatedVersion(true)>=400);
}
...
function DoJump( optional float F )
{
	...
	if ( (!bIsCrouching || Level.bSupportsCrouchJump) && (Physics == PHYS_Walking) )
	{
		if ( Role == ROLE_Authority )
		{
			...
			else PlaySound(JumpSound, SLOT_Talk, 1.5, true, 1200, 1.0 );
		}
...
event UpdateEyeHeight(float DeltaTime)
{
	...
	UpdateFOVAngle(DeltaTime);
}

simulated final function UpdateFOVAngle( float DeltaTime )
{
	// teleporters affect your FOV, so adjust it back down
	if ( FOVAngle!=DesiredFOV && (Physics!=PHYS_Interpolating || (DesiredFOV!=DefaultFOV)) )
	{
		if ( FOVAngle > DesiredFOV )
			FOVAngle = FMax(DesiredFOV, FOVAngle - DeltaTime * FMax(7 * 60, 0.9 * (FOVAngle - DesiredFOV)));
		else
			FOVAngle = FMin(DesiredFOV, FOVAngle + DeltaTime * FMax(7 * 60, 0.9 * (DesiredFOV - FOVAngle)));
	}
...
function CalcBehindView(out vector CameraLocation, out rotator CameraRotation, float Dist)
{
	...
	if ( (bool(ViewTarget) ? ViewTarget : Self).Trace( HitLocation, HitNormal, CameraLocation - (Dist + 30) * vector(CameraRotation), CameraLocation,,vect(5,5,5),,,{TRACE_AllColliding | TRACE_Blocking | TRACE_IgnoreHidden}) )
