...
function ProcessHitScan( LevelInfo TraceLevel, Actor HitActor, const out vector TraceStart, const out vector HitLocation, const out vector HitNormal, const out vector TraceDir, optional bool bFinalHit )
{
	...
	if( HitActor )
	{
		...
		{
			if( FRand() < 0.5 )
				WallHit = TraceLevel.Spawn( class'CARWallHitEffect2',,, HitLocation + HitNormal * 9, Rotator( HitNormal ) );
			else WallHit = TraceLevel.Spawn( class'CARWallHitEffect',,, HitLocation + HitNormal * 9, Rotator( HitNormal ) );
		...
		else if( !HitActor.bIsPawn && !HitActor.IsA( 'Carcass' ) )
		{
			if( FRand() < 0.01 )
			{
				WallHit = TraceLevel.Spawn( class'SpriteSmokePuff',,, HitLocation + HitNormal * 9 );
				WallHit.DrawScale -= FRand();
			}
		}
		...
		if( HitActor.IsA( 'Carcass' ) )
			rndDam = 10 + Rand(10);
		
		if( Instigator.IsA( 'SpaceMarine' ) )
		{
			rndDam = 3;
			HitActor.TakeDamage( rndDam, Instigator, HitLocation, rndDam * 500.0 * TraceDir, 'shot' );
		}
		else HitActor.TakeDamage( rndDam, Instigator, HitLocation, rndDam * 250.0 * TraceDir, 'shot' );
	}
}
