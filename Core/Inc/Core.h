...
//enum {UTF8_BOM      = 0xefbbbf   };
enum {UTF16_BOM     = 0xfeff     };
//enum {UTF16LE_BOM   = 0xfffe     };
...
enum // GUglyHackFlags
{
	...
	HACKFLAGS_AllowPropClone	= (1<<2), // Allow specific properties to clone themselves.
	...
	HACKFLAGS_DuplicateActor	= (1<<5), // Duplicating actor, don't spawn new archetypes to previous dupe.
	HACKFLAGS_DrawHiddenActor	= (1<<6), // Force URender::DrawActorSprite to draw bHidden actor attachments.
};
...
public:
	...
	virtual UPlayer* GetUserContext() const { return nullptr; } // Client that requested this output (for consolecommands).
...
#include "UnDebugLogger.h"	// Safe debug logging.
#include "UnThread.h"		// Unreal Mulithreading implementation.
...
// Multi threading:
CORE_API extern FThreadLock				GMeshMutex;
