...
public:
	...
	void Precache( INT HintCount )
	{
		...
		{
			ArIsError = 1;
			Error->Logf( TEXT("ReadFile failed: Count=%i BufferCount=%i Error=%ls"), Count, BufferCount, appGetSystemErrorMessage() );
		}
		unguardSlow;
	}
	void Seek( INT InPos )
	{
		...
		{
			ArIsError = 1;
			Error->Logf( TEXT("SetFilePointer Failed %i/%i: %i %ls"), InPos, Size, Pos, appGetSystemErrorMessage() );
		}
		Pos         = InPos;
		BufferBase  = Pos;
		BufferCount = 0;
		...
		while( Length>0 )
		{
			...
			if( Copy==0 )
			{
				if( Length >= ARRAY_COUNT(Buffer) )
				{
					...
					{
						ArIsError = 1;
						Error->Logf( TEXT("ReadFile failed: Count=%i Length=%i Error=%ls"), Count, Length, appGetSystemErrorMessage() );
					}
					Pos += Length;
					BufferBase += Length;
					return;
...
public:
	...
	void Seek( INT InPos )
	{
		...
		{
			ArIsError = 1;
			Error->Logf( *LocalizeError(TEXT("SeekFailed"),TEXT("Core")) );
		}
		Pos = InPos;
	}
	INT Tell()
	...
	UBOOL Close()
	{
		...
		{
			ArIsError = 1;
			Error->Logf( *LocalizeError(TEXT("WriteFailed"),TEXT("Core")) );
		}
		else
		{
			Handle = NULL;
...
{
public:
	FArchive* CreateFileReader( const TCHAR* Filename, DWORD Flags, FOutputDevice* InError )
	{
		...
		if (Handle == INVALID_HANDLE_VALUE)
		{
			if (Flags & FILEREAD_NoFail)
				appErrorf(TEXT("Failed to read file: %ls"), Filename);
			return NULL;
		}
		return new FArchiveFileReader(Handle, InError, GetFileSize(Handle, NULL));
		unguard;
	}
	FArchive* CreateFileWriter( const TCHAR* Filename, DWORD Flags, FOutputDevice* InError )
	{
		...
		if (Handle == INVALID_HANDLE_VALUE)
		{
			if (Flags & FILEWRITE_NoFail)
				appErrorf(TEXT("Failed to create file: %ls"), Filename);
			return NULL;
		}
		if( Flags & FILEWRITE_Append )
			Pos = SetFilePointer( Handle, 0, 0, FILE_END );
		return new FArchiveFileWriter(Handle, InError, Pos);
		unguard;
	}
	...
	UBOOL Move( const TCHAR* Dest, const TCHAR* Src, UBOOL Replace=1, UBOOL EvenIfReadOnly=0, UBOOL Attributes=0 )
	{
		...
		if( !Result )
			debugf( NAME_Warning, TEXT("Error moving file '%ls' to '%ls' (error: %d)"), Src, Dest, GetLastError());
		return Result!=0;
		unguard;
	}
