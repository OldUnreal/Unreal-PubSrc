...
#define SER_TAG_OBJ(ar,ptr) ar.SerializeTaggedObject(*reinterpret_cast<UObject**>(&ptr),TEXT(#ptr))
#define SER_TAG_OBJ_CUSTOM(ar,ptr,tag) ar.SerializeTaggedObject(*reinterpret_cast<UObject**>(&ptr),TEXT(#tag))
#define SER_TAG_NAME(ar,ptr) ar.SerializeTaggedName(ptr,TEXT(#ptr))
...
public:
	...
	virtual void SerializeTaggedName(class FName& N, const TCHAR* Tag) // For gc
	{
		*this << N;
	}
	...
	virtual void SerializeTaggedObject(class UObject*& Res, const TCHAR* Tag) // For gc
	{
		*this << Res;
	}
	virtual void SetSerializeTag(const TCHAR* Tag) // For gc
	{}
	...
	UBOOL SerializeRefs()	{return ArSerializeRefs;}
	inline INT GetMaxSerializeSize() const { return ArMaxSerializeSize; }

	inline void SetError()
	{
		ArIsError = TRUE;
	}
	inline void SetCriticalError()
	{
	...
	friend FArchive& operator<<( FArchive& Ar, TCHAR& U )
	{
    ...
    CORE_API friend FArchive& operator<<( FArchive& Ar, FString& A ); //elmuerte: security fix
    CORE_API friend FArchive& operator<<( FArchive& Ar, FStringRaw& A );
protected:
	// Status variables.
	INT ArVer;
	INT ArNetVer;
	...
	UBOOL ArForClient;
	UBOOL ArForServer;
	UBOOL ArSerializeRefs; // 227j: For GarbageCollector.
	UBOOL ArIsError;
