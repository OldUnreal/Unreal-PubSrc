...
public:
	...
	void Init(UBOOL Startup)
	{
		...
		if( RealSize<=0 )
			appErrorf( TEXT("Can't find module %ls"), Wad );
		...
		// stijn: Support for digitally signed installers. See the big comment in RunSFX.cpp for additional info
#if WIN32
		if (RealSize > 4096)
		{
			TArray<BYTE> Header;
			Header.SetSize(4096);
			check(Header.Num() == 4096 && "Couldn't verify file header");
			Ar->Serialize(&Header(0), 4096);			
			
			const _IMAGE_DOS_HEADER* DosHeader = reinterpret_cast<const _IMAGE_DOS_HEADER*>(&Header(0));
			if (DosHeader->e_magic == IMAGE_DOS_SIGNATURE)
			{
				...
				{
					if (NtHeader->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_SECURITY].VirtualAddress)
					{
						// drop the digital signature
						for (INT i = 0; i < 8; ++i)
						{
							RealSize = NtHeader->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_SECURITY].VirtualAddress - i;
							HeaderPos = RealSize - ARCHIVE_HEADER_SIZE;

							INT MagicTest;
							Ar->Seek(HeaderPos);
							*Ar << MagicTest;
							if (MagicTest == ARCHIVE_MAGIC)
								break;
						}
					}
				}
		...
		*Ar << Header;
		...
		if( Ar->IsError() || Header.Magic!=ARCHIVE_MAGIC || Header.Ver!=ARCHIVE_VERSION || Header.FileSize!=RealSize )
			appErrorf( TEXT("The module %ls is incomplete -- probably due to an incomplete or failed download"), Wad );
		...
		if( Verify && FString(Wad).Right(4)!=TEXT(".exe") )
		{
			...
			for( INT i=0; i<HeaderPos; i+=sizeof(Buffer) )
			{
				...
				if( Ar->IsError() )
					appErrorf( TEXT("The module %ls can't be read"), Wad );
				CRC = appMemCrc( Buffer, Count, CRC );
			}
			if( CRC!=Header.CRC )
				appErrorf( TEXT("The module %ls is corrupt -- probably due to an incomplete or corrupt download"), Wad );
		}
