...
#ifdef _WIN32
#include <tchar.h>
//Optional ASM code, set in preprocessor in VS. Unable to build with Clang: error: Unable to lookup field reference
...
#define VERTEX_ARRAY_SIZE	8192	// vogel: better safe than sorry
#define VERTEX_ARRAY_LIMIT	8190	// Marco: VERTEX_ARRAY_SIZE but div by 3.
...
struct FCachedTexture {
	...
	static inline DWORD MakeInfoID(const FTextureInfo& Info)
	{
		return Info.USize | (Info.VSize << 12) | (Info.Format << 24);
	}
};
...
struct FGLTexCoord {
	...
	FGLTexCoord(FLOAT inU, FLOAT inV)
		: u(inU), v(inV)
	{}
};
...
struct FGLDoubleColor {
	...
	FGLDoubleColor(DWORD inColor, DWORD inSpecular)
		: color(inColor), specular(inSpecular)
	{}
};
...
typedef void (FASTCALL* FuncBufferVerts)(UOpenGLRenderDevice*, FTransTexture*, INT);
...
class UOpenGLRenderDevice : public URenderDevice {
	...
	enum tex_type_t {
		...
		TEX_TYPE_NORMAL,
		TEX_TYPE_NORMAL_RGB10A2,
		TEX_TYPE_RGBA8,
		TEX_TYPE_COMPRESSED_BC4,
		TEX_TYPE_COMPRESSED_BC5,
		TEX_TYPE_COMPRESSED_BC7,
	};
	...
	GLuint m_vpDefaultRenderingStateWithExpFog;
	GLuint m_vpDefaultRenderingStateWithExp2Fog;
	...
	GLuint m_fpDefaultRenderingStateWithExpFog;
	GLuint m_fpDefaultRenderingStateWithExp2Fog;
	...
	// Timing.
	DWORD BindCycles, ImageCycles, ComplexCycles, GouraudCycles, TileCycles, BlendCycles, GouraudPolyListCycles;
	...
	BYTE m_gpAlpha;
	bool m_gpFogEnabled;
	bool m_gpFogExpEnabled;
	bool m_gpFogExp2Enabled;

	DWORD m_curTexEnvFlags[MAX_TMUNITS];
	FTexInfo TexInfo[MAX_TMUNITS];

	FuncBufferVerts m_pBuffer3BasicVertsProc;
	FuncBufferVerts m_pBuffer3ColoredVertsProc;
	FuncBufferVerts m_pBuffer3FoggedVertsProc;

	FuncBufferVerts m_pBuffer3VertsProc;

	GLuint m_noTextureId;
	GLuint m_alphaTextureId;
	BYTE m_LastZMode, ZTrickFunc;
...
#undef RGBA_MAKE
	...
	void PrintFormat(HDC hDC, INT nPixelFormat) {
		...
		debugf(NAME_Init, TEXT("   Flags:%ls"), Flags);
	...
	void FASTCALL ConvertRGBA8888(const FMipmapBase* Mip, INT Level);
	void FASTCALL ConvertRGBA101002(const FMipmapBase* Mip, INT Level);

	inline void FASTCALL SetBlend(DWORD PolyFlags) {
		clockFast(BlendCycles);
#ifdef UTGLR_RUNE_BUILD
	...
	/*
	bool bInverseOrder;
	TMap<UMesh*,FMeshBufferData*> BufferMap;
	FMeshBufferData* FindMeshBuffer( UMesh* Mesh );
	void InitMeshBuffer( FMeshBufferData& B );
	void ExitMeshBuffer( FMeshBufferData& B );
	void PrepareBufferedMesh( AActor* Actor, const FCoords& Coords, bool bWireframe );
	void EndBufferedMesh();
	void DrawMeshBuffer( FMeshBufferData& B, INT Num, UTexture* Tex, DWORD Flags );
	void DrawMeshWireframe( FMeshBufferData& B, INT Num, FPlane Color );
	*/
};
