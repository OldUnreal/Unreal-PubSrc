...
static const char *g_vpDefaultRenderingStateWithExpFog =
	"!!ARBvp1.0\n"
	"OPTION ARB_position_invariant;\n"
...
static const char *g_vpDefaultRenderingStateWithExp2Fog =
	"!!ARBvp1.0\n"
	"OPTION ARB_position_invariant;\n"
...
static const char *g_fpDefaultRenderingStateWithExpFog =
	"!!ARBfp1.0\n"
	"OPTION ARB_fog_exp;\n"
...
static const char *g_fpDefaultRenderingStateWithExp2Fog =
	"!!ARBfp1.0\n"
	"OPTION ARB_fog_exp2;\n"
...
void UOpenGLRenderDevice::SC_AddByteConfigParam(const TCHAR *pName, BYTE &param, ECppProperty EC_CppProperty, INT InOffset, BYTE defaultValue) {
	param = defaultValue;
	new(GetClass(), pName, RF_Public)UByteProperty(EC_CppProperty, InOffset, TEXT("Options"), CPF_Config);
}

void UOpenGLRenderDevice::SC_AddEnumConfigParam(const TCHAR *pName, BYTE &param, ECppProperty EC_CppProperty, INT InOffset, UEnum* Enum, BYTE defaultValue) {
	param = defaultValue;
	...
	for (INT i = 0; i < 3; i++) {
		...
		if (pRD->m_requestedColorFlags & UOpenGLRenderDevice::CF_DUAL_COLOR_ARRAY) {
			...
			if (pRD->m_gpAlpha > 0)
				...
				pSingleColorArray->color = UOpenGLRenderDevice::FPlaneTo_RGBAClamped(&P->Light);
#else
...
static void FASTCALL Buffer3BasicVerts(UOpenGLRenderDevice *pRD, FTransTexture* Pts, INT NumPts)
{
	FGLTexCoord *pTexCoordArray = &pRD->TexCoordArray[0][pRD->BufferedVerts];
	FGLVertex *pVertexArray = &pRD->VertexArray[pRD->BufferedVerts];
	pRD->BufferedVerts += NumPts;
	const FLOAT UMult = pRD->TexInfo[0].UMult;
	const FLOAT VMult = pRD->TexInfo[0].VMult;
	for (INT i = 0; i < NumPts; ++i, ++Pts)
	{
		*pTexCoordArray++ = FGLTexCoord(Pts->U * UMult, Pts->V * VMult);
		*pVertexArray++ = FGLVertex(Pts->Point);
	}
}

static void FASTCALL Buffer3ColoredVerts(UOpenGLRenderDevice* pRD, FTransTexture* Pts, INT NumPts)
{
	FGLTexCoord *pTexCoordArray = &pRD->TexCoordArray[0][pRD->BufferedVerts];
	FGLVertex *pVertexArray = &pRD->VertexArray[pRD->BufferedVerts];
	FGLSingleColor *pSingleColorArray = &pRD->SingleColorArray[pRD->BufferedVerts];
	...
	for (INT i = 0; i < NumPts; ++i, ++Pts)
	{
		*pTexCoordArray++ = FGLTexCoord(Pts->U * UMult, Pts->V * VMult);
		*pVertexArray++ = FGLVertex(Pts->Point);
		*pSingleColorArray++ = FGLSingleColor(UOpenGLRenderDevice::FPlaneTo_RGB_A255(&Pts->Light));
	}
}

static void FASTCALL Buffer3ColoredVertsAlpha(UOpenGLRenderDevice* pRD, FTransTexture* Pts, INT NumPts)
{
	FGLTexCoord* pTexCoordArray = &pRD->TexCoordArray[0][pRD->BufferedVerts];
	FGLVertex* pVertexArray = &pRD->VertexArray[pRD->BufferedVerts];
	FGLSingleColor* pSingleColorArray = &pRD->SingleColorArray[pRD->BufferedVerts];
	...
	for (INT i = 0; i < NumPts; ++i, ++Pts)
	{
		*pTexCoordArray++ = FGLTexCoord(Pts->U * UMult, Pts->V * VMult);
		*pVertexArray++ = FGLVertex(Pts->Point);
		*pSingleColorArray++ = FGLSingleColor(UOpenGLRenderDevice::FPlaneTo_RGBA(&Pts->Light));
	}
}

static void FASTCALL Buffer3FoggedVerts(UOpenGLRenderDevice* pRD, FTransTexture* Pts, INT NumPts)
{
	FGLTexCoord* pTexCoordArray = &pRD->TexCoordArray[0][pRD->BufferedVerts];
	FGLVertex* pVertexArray = &pRD->VertexArray[pRD->BufferedVerts];
	FGLDoubleColor* pDoubleColorArray = &pRD->DoubleColorArray[pRD->BufferedVerts];
	...
	for (INT i = 0; i < NumPts; ++i, ++Pts)
	{
		*pTexCoordArray++ = FGLTexCoord(Pts->U * UMult, Pts->V * VMult);
		*pVertexArray++ = FGLVertex(Pts->Point);
		*pDoubleColorArray++ = FGLDoubleColor(UOpenGLRenderDevice::FPlaneTo_RGBScaled_A255(&Pts->Light, 255.0f * (1.0f - Pts->Fog.W)), UOpenGLRenderDevice::FPlaneTo_RGB_A0(&Pts->Fog));
	}
}
	...
	do {
		...
		if (m_requestedColorFlags & CF_DUAL_COLOR_ARRAY) {
			...
			if (m_gpAlpha > 0)
				...
				SingleColorArray[BufferedVerts].color = FPlaneTo_RGBAClamped(&P->Light);
#else
...
void UOpenGLRenderDevice::SetGamma(FLOAT GammaCorrection) {
	...
	if (Window)
    {
        if (SDL_SetWindowGammaRamp(Window, gammaRamp.red, gammaRamp.green, gammaRamp.blue )< 0)
            debugf(TEXT("SDL_SetWindowGammaRamp failed"));
    }
#else
...
#endif
	if (!ProcAddress) {
		debugf(TEXT("   Missing function '%ls' for '%ls' support"), appFromAnsi(pName), TEXT("OpenGL 1.x"));
		dbgPrintf("Missing function '%ls' for '%ls' support\n", pName, "OpenGL 1.x");
		return false;
	}
...
bool UOpenGLRenderDevice::FindGLExt(const char *pName) {
	...
	if (bRet) {
		debugf(NAME_Init, TEXT("Device supports: %ls"), appFromAnsi(pName));
	}
	if (DebugBit(DEBUG_BIT_BASIC)) {
		dbgPrintf("utglr: GL_EXT: %ls = %d\n", pName, bRet);
	}
...
void UOpenGLRenderDevice::GetGLExtProc(void*& ProcAddress, const char *pName, const char *pSupportName, bool& Supports) {
	...
	if (!ProcAddress) {
		...
		debugf(TEXT("   Missing function '%ls' for '%ls' support"), appFromAnsi(pName), appFromAnsi(pSupportName));
		dbgPrintf("Missing function '%ls' for '%ls' support\n", pName, pSupportName);
	}
...
void UOpenGLRenderDevice::GetGLExtProcs(void) {
    ...
    #define GL_EXT_NAME(name) SUPPORTS##name = FindGLExt((const char*)#name + 1);
    #define GL_EXT_PROC(ext, ret, func, params) GetGLExtProc(*(void**)&func, #func, (const char*)#ext + 1, SUPPORTS##ext);
...
UBOOL UOpenGLRenderDevice::SetRes(INT NewX, INT NewY, INT NewColorBytes, UBOOL Fullscreen) {
	...
	if (UseAA) {
		...
		SDL_GL_SetAttribute(SDL_GL_MULTISAMPLESAMPLES,NumAASamples);
        m_usingAA = true;
	...
	if (m_hRC && !Fullscreen && !WasFullscreen && NewColorBytes == (INT)Viewport->ColorBytes) {
	...
	if (strlen(pGLExtensions) < 1024) {
		debugf(NAME_Init, TEXT("GL_EXTENSIONS : %ls"), appFromAnsi(pGLExtensions));
	}
	else {
#ifdef SDLBUILD
		printf("GL_EXTENSIONS : %ls\n", appFromAnsi(pGLExtensions));
#elif SDL2BUILD
		printf("GL_EXTENSIONS : %ls\n", appFromAnsi(pGLExtensions));
#endif
	}
...
//		UTGLR_DEBUG_SHOW_PARAM_REG(FrameRateLimit);
	...
	UnsupportHDLightFlags = PF_Masked; // TODO - Fix masked BSP textures with this. Most likely related to glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA) and glAlphaFunc(GL_GREATER, 0.5f);
	...
	VertexArray = (FGLVertex *)Align(m_VertexArrayMem, VERTEX_ARRAY_ALIGN);
	NormalArray = (FGLNormal *)Align(m_NormalArrayMem, VERTEX_ARRAY_ALIGN);
	for (u = 0; u < MAX_TMUNITS; u++) {
		TexCoordArray[u] = (FGLTexCoord *)Align(m_TexCoordArrayMem[u], VERTEX_ARRAY_ALIGN);
	}
	MapDotArray = (FGLMapDot *)Align(m_MapDotArrayMem, VERTEX_ARRAY_ALIGN);
	SingleColorArray = (FGLSingleColor *)Align(m_ColorArrayMem, VERTEX_ARRAY_ALIGN);
	DoubleColorArray = (FGLDoubleColor *)Align(m_ColorArrayMem, VERTEX_ARRAY_ALIGN);
	...
	m_gpFogExpEnabled = false;
	m_gpFogExp2Enabled = false;
...
#if !defined(SDLBUILD) && !defined(SDL2BUILD)
	check(m_hRC)
#endif
...
void UOpenGLRenderDevice::CheckGLErrorFlag(const TCHAR *pTag) {
	...
	if ((Error != GL_NO_ERROR) && DebugBit(DEBUG_BIT_GL_ERROR)) {
		...
		//appErrorf(TEXT("OpenGL Error: %ls (%ls)"), pMsg, pTag);
		debugf(TEXT("OpenGL Error: %ls (%ls)"), pMsg, pTag);
	}
}
...
void UOpenGLRenderDevice::InitARBPixelFormat(INT NewColorBytes, InitARBPixelFormatRet_t *pRet) {
	...
	HINSTANCE hOGLInstance = GetModuleHandleW(NULL);
	...
	wcA.hInstance = wcW.hInstance = hOGLInstance;
	wcA.hIcon = wcW.hIcon = NULL;
	wcA.hCursor = wcW.hCursor = LoadCursorW(NULL, MAKEINTRESOURCEW(32512)/*IDC_ARROW*/);
	...
	hWnd =
		...
		&initParams);
	if (hWnd == NULL) {
		UnregisterClassW(pClassNameW, hOGLInstance);
		return;
	}
...
void UOpenGLRenderDevice::SetBasicPixelFormat(INT NewColorBytes) {
	...
	check(SetPixelFormat(m_hDC, nPixelFormat, &pfd));
...
bool UOpenGLRenderDevice::SetAAPixelFormat(INT NewColorBytes) {
	...
	check(SetPixelFormat(m_hDC, iFormats[0], &tempPfd));
	m_hRC = wglCreateContext(m_hDC);
	check(m_hRC);
...
UBOOL UOpenGLRenderDevice::Init(UViewport* InViewport, INT NewX, INT NewY, INT NewColorBytes, UBOOL Fullscreen) {
	...
	debugf(TEXT("NoAATiles %ls"),(NoAATiles ? TEXT("True") : TEXT("False")));
	debugf(TEXT("ZRangeHack %ls"),(ZRangeHack ? TEXT("True") : TEXT("False")));
	debugf(TEXT("UseHardwareClipping %ls"),(UseHardwareClipping ? TEXT("True") : TEXT("False")));
	*/
	...
	if (NumDevices == 0) {
		...
		#elif _MSC_VER
		if (!GConfig->GetString(g_pSection, TEXT("OpenGLLibName"), OpenGLLibName)) {
			OpenGLLibName = TEXT("opengl32.dll");
		}
		#elif MACOSX || MACOSXPPC
		// Default to libGL.dylib if not defined
		if (!GConfig->GetString(g_pSection, TEXT("OpenGLLibName"), OpenGLLibName)) {
		...
		if (!GLLoaded) {
			...
			debugf(TEXT("binding %ls"), *OpenGLLibName);
			#if UNICODE
			if (SDL_GL_LoadLibrary(appToAnsi(*OpenGLLibName)) == -1) {
				appErrorf(TEXT("%ls"),SDL_GetError());
            #else
	...
	// Init global GL.
	// no need to do SDL_Init here, done in SDLDrv (USDL2Client::Init).
	INT MinDepthBits=0;
	if (!GConfig->GetInt(g_pSection, TEXT("MinDepthBits"), MinDepthBits)) MinDepthBits = 16;
	...
	if (NumDevices == 0) {
		...
		if (!hModuleGlMain) {
			debugf(NAME_Init, *LocalizeError(TEXT("NoFindGL")), appFromAnsi(GL_DLL));
			return 0;
		}
...
UBOOL UOpenGLRenderDevice::Exec(const TCHAR* Cmd, FOutputDevice& Ar) {
...
#ifdef SDLBUILD
			debugf(TEXT("OpenGL renderer built: %ls %ls"), __DATE__, __TIME__);
#else
	...
	else if (ParseCommand(&Cmd, TEXT("GetRes")))
	{
		...
		if (modes == (SDL_Rect **)0) {
			...
			// to sm...
			for (i = (j - 1); i >= 0; --i)
				Relevant.AddUniqueItem(FPlane(modes[i]->w, modes[i]->h, 0, 0));
		}
#elif SDL2BUILD
		// Available fullscreen video modes
		...
		//debugf(TEXT("SDL_GetNumVideoDisplays returned: %i"), display_count);
		 ...
		 for(i = 0; i < display_count; i++)
		 {
			...
			else Relevant.AddUniqueItem(FPlane(mode.w, mode.h, 0, 0));
		}
#else
...
void UOpenGLRenderDevice::Lock(FPlane InFlashScale, FPlane InFlashFog, FPlane ScreenClear, DWORD RenderLockFlags, BYTE* InHitData, INT* InHitSize) {
	...
	if (!UseZTrick || GIsEditor || (RenderLockFlags & LOCKR_ClearScreen)) {
		...
		ZTrickFunc = ZTEST_GreaterEqual;
		glPolygonOffset(1.0f, 1.0f);
	}
	m_LastZMode = 255;
	SetZTestMode(ZTrickFunc);
...
#if 0 // Marco: Must rewrite ASM functions...
...
void UOpenGLRenderDevice::SetSceneNode(FSceneNode* Frame) {
	...
	// Disable clipping
	while (m_NumClipPlanes > 0)
	{
		--m_NumClipPlanes;
		glDisable(GL_CLIP_PLANE0 + m_NumClipPlanes);
	}
...
void UOpenGLRenderDevice::DrawComplexSurface(FSceneNode* Frame, FSurfaceInfo& Surface, FSurfaceFacet& Facet) {
	...
	if (Surface.MacroTexture) //exclude HeightMaps from being rendered as MacroTexture
    {
...
void UOpenGLRenderDevice::PreDrawGouraud(FSceneNode* Frame, FFogSurf &FogSurf) {
	UTGLR_DEBUG_CALL_COUNT(PreDrawGouraud);
	guard(UOpenGLRenderDevice::PreDrawGouraud);
	...
	//Enable fog
	//Unfortunately these fixed functions won't work anymore once a fragment shader is active.
...
void UOpenGLRenderDevice::PostDrawGouraud(FSceneNode* Frame, FFogSurf &FogSurf) {
	UTGLR_DEBUG_CALL_COUNT(PostDrawGouraud);
	guard(UOpenGLRenderDevice::PostDrawGouraud);
	EndBuffering();
	...
	if (UseFragmentProgram)
	{
		...
		{
			PreDrawFogSurface();
			DrawFogSurface(Frame, FogSurf); //Unlike GL_FOG this has to be done AFTER DrawComplexSurface as an additional pass. SLOW!
			SetBlend(0);
		}
		m_gpFogEnabled = false;
		m_gpFogExpEnabled = false;
...
void UOpenGLRenderDevice::DrawGouraudPolyList( FSceneNode* Frame, FTextureInfo& Info, FTransTexture* Pts, int NumPts, DWORD PolyFlags, FSpanBuffer* Span )
{
	guard(UOpenGLRenderDevice::DrawGouraudPolyList);
	...
	if ( m_bufferActorTrisCutoff<3 )
	{
		...
		if (UseFragmentProgram && (m_gpFogEnabled || m_gpFogExpEnabled || m_gpFogExp2Enabled))
		{
			if(m_gpFogEnabled)
				SetShaderState(m_vpDefaultRenderingStateWithLinearFog, m_fpDefaultRenderingStateWithLinearFog);
			else if(m_gpFogExpEnabled)
				SetShaderState(m_vpDefaultRenderingStateWithExpFog, m_fpDefaultRenderingStateWithExpFog);
			else if(m_gpFogExp2Enabled)
				SetShaderState(m_vpDefaultRenderingStateWithExp2Fog, m_fpDefaultRenderingStateWithExp2Fog);
		}
		else SetDefaultShaderState();
		SetDefaultTextureState();
		...
		INT Count = NumPts/3;
		FTransTexture* Ptz = Pts;
		INT i, j;
		const FLOAT uMulti = TexInfo[0].UMult, vMulti = TexInfo[0].VMult;
		if (m_requestedColorFlags & CF_COLOR_ARRAY)
		{
			if (PolyFlags & PF_AlphaBlend)
			{
				for (i = 0; i < Count; i += 3, Ptz += 3)
				{
					for (j = 0; j < 3; ++j)
					{
						TexCoordArray[0][j] = FGLTexCoord(Ptz[j].U * uMulti, Ptz[j].V * vMulti);
						SingleColorArray[j].color = FPlaneTo_RGBA(&Ptz[j].Light);
					}
			...
			else
			{
				for (i = 0; i < Count; i += 3, Ptz += 3)
				{
					...
					{
						TexCoordArray[0][j] = FGLTexCoord(Ptz[j].U * uMulti, Ptz[j].V * vMulti);
						SingleColorArray[j].color = FPlaneTo_RGB_A255(&Ptz[j].Light);
					}
					glDrawArrays(GL_TRIANGLE_FAN, 0, 3);
				}
			}
		...
		if( (PolyFlags & (PF_RenderFog | PF_Translucent | PF_Modulated | PF_AlphaBlend)) == PF_RenderFog )
		{
			...
			{
				for (j = 0; j < 3; ++j)
					SingleColorArray[j].color = FPlaneTo_RGBA(&Ptz[j].Fog);
				glDrawArrays(GL_TRIANGLE_FAN, 0, 3);
			}
		}
		return;
	...
	if ((m_curPolyFlags != PolyFlags) ||
		...
		((BufferedVerts + 3) >= (VERTEX_ARRAY_SIZE - 14)) ||
		(BufferedVerts == 0))
	{
		...
		else if (UseFragmentProgram && (m_requestedColorFlags & CF_DUAL_COLOR_ARRAY))
		{
		...
		{
			m_pBuffer3VertsProc = (PolyFlags & PF_AlphaBlend) ? Buffer3ColoredVertsAlpha : Buffer3ColoredVerts;
		}
		else
		{
			m_pBuffer3VertsProc = Buffer3BasicVerts;
		}
	...
	if (BufferTileQuads) {
		...
		if (!(PolyFlags & PF_Modulated)) {
			if (UseSSE2) {
				...
				if (PolyFlags & PF_AlphaBlend)
					dwColor = FPlaneTo_RGBAClamped(&Color);
				else dwColor = FPlaneTo_RGBClamped_A255(&Color);
#else
				dwColor = FPlaneTo_RGBClamped_A255(&Color);
#endif
		...
		if (Info.Modifier)
		{
			U = SU1;
			V = SV1;
			Info.Modifier->TransformPoint(U, V);
			glTexCoord2f(U, V);
...
void UOpenGLRenderDevice::SetTextureNoCheck(FTexInfo& Tex, FTextureInfo& Info, DWORD PolyFlags) {
	...
	if (pBind)
	{
		if (pBind->TexInfoID != FCachedTexture::MakeInfoID(Info))
		{
			debugfSlow(TEXT("OpenGL Error: Found 2 different textures using same CacheID."));
			bool isZeroPrefixCacheID = ((Tex.CurrentCacheID & 0xFFFFFFFF00000000ULL) == 0) ? true : false;
			glDeleteTextures(1, &pBind->Id);
...
void UOpenGLRenderDevice::UploadTextureExec(FTextureInfo& Info, DWORD PolyFlags, FCachedTexture *pBind, bool existingBind, bool needTexAllocate) {
	...
	for (Level = 0; Level <= MaxUploadLevel; Level++) {
		...
		if (!Mip || !Mip->DataPtr) {
			...
			case TEX_TYPE_COMPRESSED_BC4:
				//No conversion required for compressed BC4 textures
				break;

			case TEX_TYPE_COMPRESSED_BC5:
				//No conversion required for compressed BC5 textures
				break;

			case TEX_TYPE_COMPRESSED_BC7:
				//No conversion required for compressed BC7 textures
				break;

			case TEX_TYPE_RGBA8:
				//No conversion required for RGBA8 textures
				break;

			case TEX_TYPE_NORMAL_RGB10A2:
...
#if 0
{
	...
	dbgPrintf("utglr: glCompressedTexImage2DARB = %u\n", s_c++);
					...
					glCompressedTexImage2DARB(
						...
						(pBind->texType == TEX_TYPE_COMPRESSED_DXT1_TO_DXT3) ? Src : Mip->DataPtr);
						/*
						debugf(TEXT("TEXF_DXT1 %ls Level %i, MaxUploadLevel %i, UBits %i VBits %i, ImageSize %i, Info.NumMips %i, InternalFormat %x"), Info.Texture->GetName(),
						Level,
						MaxUploadLevel,
						texWidth,
						texHeight,
...
BYTE UOpenGLRenderDevice::SetZTestMode(BYTE Mode)
{
...
}
{
	...
	if (pName) dbgPrintf("utglr: TexName = %ls\n", appToAnsi(pName));
}
{
...
}
{
	...
	if (SupportsTC) {
		...
		case TEXF_BC3:
            ...
            break;
		case TEXF_BC4:
			pBind->texType = TEX_TYPE_COMPRESSED_BC4;
			//texSourceFormat not used for compressed textures
			pBind->texInternalFormat = GL_COMPRESSED_RED_RGTC1;
			break;
		case TEXF_BC5:
			pBind->texType = TEX_TYPE_COMPRESSED_BC5;
			//texSourceFormat not used for compressed textures
			pBind->texInternalFormat = GL_COMPRESSED_RG_RGTC2;
			break;
		case TEXF_BC7:
			pBind->texType = TEX_TYPE_COMPRESSED_BC7;
			//texSourceFormat not used for compressed textures
			pBind->texInternalFormat = GL_COMPRESSED_RGBA_BPTC_UNORM;
			break;
#endif
	...
	if (Info.Format == TEXF_RGB10A2_LM)
	{
		...
		pBind->texInternalFormat = GL_RGB10_A2;
	}
		...
		if (UseBGRATextures) {
			...
			if (texFlags & TEX_FLAG_NO_CLAMP) {
				pBind->pConvertBGRA7777 = (Info.Format == TEXF_BGRA8) ? &UOpenGLRenderDevice::ConvertBGRA8888_NoClamp : &UOpenGLRenderDevice::ConvertBGRA7777_BGRA8888_NoClamp;
			}
			else {
				pBind->pConvertBGRA7777 = (Info.Format == TEXF_BGRA8) ? &UOpenGLRenderDevice::ConvertBGRA8888 : &UOpenGLRenderDevice::ConvertBGRA7777_BGRA8888;
			}
		}
		else {
			pBind->texSourceFormat = GL_RGBA;
			pBind->pConvertBGRA7777 = (Info.Format == TEXF_BGRA8) ? &UOpenGLRenderDevice::ConvertRGBA8888 : &UOpenGLRenderDevice::ConvertBGRA7777_RGBA8888;
		}
		pBind->texInternalFormat = GL_RGBA8;
	}
...
void UOpenGLRenderDevice::ConvertBGRA7777_BGRA8888(const FMipmapBase *Mip, INT Level) {
	DWORD* Ptr = (DWORD*)m_texConvertCtx.pCompose;
	INT StepBits = m_texConvertCtx.stepBits;
	DWORD VMask = Mip->VSize - 1;
	...
	INT i = 0;
	do { //i_stop always >= 1
		DWORD* Base = (DWORD*)Mip->DataPtr + Min<DWORD>(i & VMask, VClampVal) * Mip->USize;
		INT j = 0;
		do { //j_stop always >= 1;
			*Ptr = Base[Min<DWORD>(j & UMask, UClampVal)] << 1; // because of 7777
			Ptr++;
		} while ((j += ij_inc) < j_stop);
	} while ((i += ij_inc) < i_stop);
...
void UOpenGLRenderDevice::ConvertBGRA7777_BGRA8888_NoClamp(const FMipmapBase *Mip, INT Level) {
	...
	INT i = 0;
	do { //i_stop always >= 1
		DWORD* Base = (DWORD*)Mip->DataPtr + (DWORD)(i & VMask) * Mip->USize;
		INT j = 0;
		do { //j_stop always >= 1;
			*Ptr = Base[(DWORD)(j & UMask)] << 1; // because of 7777
			Ptr++;
		} while ((j += ij_inc) < j_stop);
	} while ((i += ij_inc) < i_stop);
...
void UOpenGLRenderDevice::ConvertBGRA7777_RGBA8888(const FMipmapBase *Mip, INT Level) {
	...
	do { //i_stop always >= 1
		...
		INT j = 0;
		do { //j_stop always >= 1;
			const DWORD Src = Base[Min<DWORD>(j & UMask, UClampVal)];
			*Ptr = ((Src & 0xFF00FF) | ((Src & 0xFF00) << 16) | ((Src & 0xFF000000) >> 16)) << 1; // because of 7777
			Ptr++;
		} while ((j += ij_inc) < j_stop);
	} while ((i += ij_inc) < i_stop);
...
void UOpenGLRenderDevice::ConvertBGRA8888(const FMipmapBase* Mip, INT Level) {
	DWORD* Ptr = (DWORD*)m_texConvertCtx.pCompose;
	INT StepBits = m_texConvertCtx.stepBits;
	DWORD VMask = Mip->VSize - 1;
	DWORD VClampVal = m_texConvertCtx.pBind->VClampVal;
	...
	do { //i_stop always >= 1
		...
		INT j = 0;
		do { //j_stop always >= 1;
			*Ptr = Base[Min<DWORD>(j & UMask, UClampVal)];
			Ptr++;
		} while ((j += ij_inc) < j_stop);
	} while ((i += ij_inc) < i_stop);
...
void UOpenGLRenderDevice::ConvertBGRA8888_NoClamp(const FMipmapBase* Mip, INT Level) {
	DWORD* Ptr = (DWORD*)m_texConvertCtx.pCompose;
	INT StepBits = m_texConvertCtx.stepBits;
	DWORD VMask = Mip->VSize - 1;
	DWORD UMask = Mip->USize - 1;
...
void UOpenGLRenderDevice::ConvertRGBA8888(const FMipmapBase* Mip, INT Level) {
	DWORD* Ptr = (DWORD*)m_texConvertCtx.pCompose;
	INT StepBits = m_texConvertCtx.stepBits;
	DWORD VMask = Mip->VSize - 1;
	DWORD VClampVal = m_texConvertCtx.pBind->VClampVal;
	...
	do { //i_stop always >= 1
		...
		do { //j_stop always >= 1;
			const DWORD Src = Base[Min<DWORD>(j & UMask, UClampVal)];
			*Ptr = ((Src & 0xFF00FF) | ((Src & 0xFF00) << 16) | ((Src & 0xFF000000) >> 16));
			Ptr++;
		} while ((j += ij_inc) < j_stop);
	} while ((i += ij_inc) < i_stop);

	UTGLR_DEBUG_TEX_CONVERT_COUNT(ConvertRGBA8888);
}

void UOpenGLRenderDevice::ConvertRGBA101002(const FMipmapBase* Mip, INT Level) {
	DWORD* Ptr = (DWORD*)m_texConvertCtx.pCompose;
	INT StepBits = m_texConvertCtx.stepBits;
	DWORD VMask = Mip->VSize - 1;
	DWORD VClampVal = m_texConvertCtx.pBind->VClampVal;
	...
	do { //i_stop always >= 1
		...
		INT j = 0;
		do { //j_stop always >= 1;
			const DWORD dwColor = Base[Min<DWORD>(j & UMask, UClampVal)];
			*Ptr++ = ((dwColor & 0x000001FF) << 21) | ((dwColor & 0x0007FC00) << 1) | ((dwColor & 0x1FF00000) >> 20) | (dwColor & 0xC0000000);
		} while ((j += ij_inc) < j_stop);
	} while ((i += ij_inc) < i_stop);
...
void UOpenGLRenderDevice::SetBlendNoCheck(DWORD blendFlags) {
	guardSlow(UOpenGLRenderDevice::SetBlendNoCheck);
	...
	if (Xor & PF_Masked) {
#endif
		if ((blendFlags & (PF_Masked | PF_AlphaBlend)) == PF_Masked)
		{
#ifdef UTGLR_UNREAL_227_BUILD
			glAlphaFunc(GL_GREATER, 0.5f);
...
bool UOpenGLRenderDevice::LoadVertexProgram(GLuint vpId, const char *pProgram, const char *pName) {
	...
	if (DebugBit(DEBUG_BIT_BASIC)) {
		dbgPrintf("utglr: Loading vertex program \"%ls\"\n", pName);
	}

	glBindProgramARB(GL_VERTEX_PROGRAM_ARB, vpId);
	glProgramStringARB(GL_VERTEX_PROGRAM_ARB, GL_PROGRAM_FORMAT_ASCII_ARB, static_cast<GLsizei>(strlen(pProgram)), pProgram);
...
bool UOpenGLRenderDevice::LoadFragmentProgram(GLuint fpId, const char *pProgram, const char *pName) {
	...
	if (DebugBit(DEBUG_BIT_BASIC)) {
		dbgPrintf("utglr: Loading fragment program \"%ls\"\n", pName);
	}

	glBindProgramARB(GL_FRAGMENT_PROGRAM_ARB, fpId);
	glProgramStringARB(GL_FRAGMENT_PROGRAM_ARB, GL_PROGRAM_FORMAT_ASCII_ARB, static_cast<GLsizei>(strlen(pProgram)), pProgram);
...
void UOpenGLRenderDevice::AllocateFragmentProgramNamesSafe(void) {
	...
	glGenProgramsARB(1, &m_vpDefaultRenderingStateWithExpFog);
	glGenProgramsARB(1, &m_vpDefaultRenderingStateWithExp2Fog);
	...
	glGenProgramsARB(1, &m_fpDefaultRenderingStateWithExpFog);
	glGenProgramsARB(1, &m_fpDefaultRenderingStateWithExp2Fog);
...
void UOpenGLRenderDevice::FreeFragmentProgramNamesSafe(void) {
	...
	glDeleteProgramsARB(1, &m_vpDefaultRenderingStateWithExpFog);
	glDeleteProgramsARB(1, &m_vpDefaultRenderingStateWithExp2Fog);
	...
	glDeleteProgramsARB(1, &m_fpDefaultRenderingStateWithExpFog);
	glDeleteProgramsARB(1, &m_fpDefaultRenderingStateWithExp2Fog);
...
bool UOpenGLRenderDevice::InitializeFragmentPrograms(void) {
	...
	//Default rendering state with exp fog
	initOk &= LoadVertexProgram(m_vpDefaultRenderingStateWithExpFog, g_vpDefaultRenderingStateWithExpFog,
		"Default rendering state with exp fog");

	//Default rendering state with exp2 fog
	initOk &= LoadVertexProgram(m_vpDefaultRenderingStateWithExp2Fog, g_vpDefaultRenderingStateWithExp2Fog,
		"Default rendering state with exp2 fog");
	...
	initOk &= LoadFragmentProgram(m_fpDefaultRenderingStateWithExpFog, g_fpDefaultRenderingStateWithExpFog,
		"Default rendering state with exp fog");

	initOk &= LoadFragmentProgram(m_fpDefaultRenderingStateWithExp2Fog, g_fpDefaultRenderingStateWithExp2Fog,
		"Default rendering state with exp2 fog");
...
void UOpenGLRenderDevice::SetOrthoProjection(void) {
	...
	glOrtho(-m_RProjZ, +m_RProjZ, -m_Aspect*m_RProjZ, +m_Aspect*m_RProjZ, 1.0 * 0.5, 32768.0);
...
void UOpenGLRenderDevice::DrawStats(FSceneNode* Frame)
{
	guard(UOpenGLRenderDevice::DrawStats);
	...
	Frame->Viewport->Canvas->WrappedPrintf(Frame->Viewport->Canvas->MedFont, 0, TEXT("[OpenGL]"));
	...
	//if (NVIDIAMemoryInfo)
	{
		...
		Frame->Viewport->Canvas->CurX = 400;
		Frame->Viewport->Canvas->CurY = (CurY += 12);
		Frame->Viewport->Canvas->WrappedPrintf(Frame->Viewport->Canvas->MedFont, 0, TEXT("NVidia VRAM (%d MB) Used (%d MB) Usage: %f%%"), TotalAvailableVideoMemory / 1024, (TotalAvailableVideoMemory - CurrentAvailableVideoMemory) / 1024, 100.0f - Percent);
		glGetError();
	}
	//if (AMDMemoryInfo)
	{
		...
		Frame->Viewport->Canvas->CurX = 400;
		Frame->Viewport->Canvas->CurY = (CurY += 12);
		Frame->Viewport->Canvas->WrappedPrintf(Frame->Viewport->Canvas->MedFont, 0, TEXT("AMD CurrentAvailableTextureMemory (%d MB) CurrentAvailableVBOMemory (%d MB) CurrentAvailableRenderbufferMemory (%d MB)"), CurrentAvailableTextureMemory / 1024, CurrentAvailableVBOMemory / 1024, CurrentAvailableRenderbufferMemory / 1024);
		glGetError();
	}
#endif
...
// Clipping planes!
BYTE UOpenGLRenderDevice::PushClipPlane(const FPlane& Plane)
{
	if (m_NumClipPlanes == GL_MAX_CLIP_PLANES)
		return 2;

	EndBuffering();
	GLdouble GLP[4] = { Plane.X,Plane.Y,Plane.Z,-Plane.W };
	glClipPlane(GL_CLIP_PLANE0 + m_NumClipPlanes, GLP);
	glEnable(GL_CLIP_PLANE0 + m_NumClipPlanes);
	++m_NumClipPlanes;
	return 1;
}
BYTE UOpenGLRenderDevice::PopClipPlane()
{
