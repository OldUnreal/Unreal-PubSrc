...
#exec MESH SEQUENCE MESH=ASMDPick SEQ=All   STARTFRAME=0  NUMFRAMES=1
#exec TEXTURE IMPORT NAME=ASMD1 FILE=Models\ASMD.pcx GROUP="Skins"
...
var transient Actor EffectLevel;
...
function inventory SpawnCopy( pawn Other )
{
	...
	foreach Other.AllInventory(class'Amplifier',A)
	{
		ASMD(Copy).Amp = A;
		break;
	}
...
{
	TraceHitScanShot(Other, HitLocation, HitNormal, X, Owner.Location + CalcDrawOffset() + FireOffset.X * X + FireOffset.Y * 3.3 * Y + FireOffset.Z * Z * 3.0);
}
...
function ProcessHitScan( LevelInfo TraceLevel, Actor HitActor, const out vector TraceStart, const out vector HitLocation, const out vector HitNormal, const out vector TraceDir, optional bool bFinalHit )
{
	...
	EffectLevel = TraceLevel;
	
	DVector = HitLocation - TraceStart;
	NumPoints = Min(VSize(DVector) / 70.f,15);
	if( NumPoints )
		SpawnEffect(DVector, NumPoints, rotator(DVector), TraceStart + (DVector/NumPoints));
	
	if( bFinalHit )
	{
		Mult = bool(Amp) ? Amp.UseCharge(100) : 1.f;
		if( HitActor )
		{
			if ( TazerProj(HitActor) )
			{
				AmmoType.UseAmmo(2);
				HitActor.Instigator = Pawn(Owner);
				TazerProj(HitActor).SuperExplosion();
				return;
			}
			else HitActor.TakeDamage(HitDamage*Mult, Instigator, HitLocation, 50000.0*TraceDir, 'jolted');
		}
		if( Mult>1.5 )
			rc = class'RingExplosion3';
		else rc = class'RingExplosion';
		r = TraceLevel.Spawn(rc,,, HitLocation+HitNormal*8,rotator(HitNormal));
		if ( r )
...
function SpawnEffect(Vector DVector, int NumPoints, rotator SmokeRotation, vector SmokeLocation)
{
	...
	if( !EffectLevel )
		EffectLevel = Self;
	Smoke = EffectLevel.Spawn(class'RingExplosion4',,,SmokeLocation,SmokeRotation);
