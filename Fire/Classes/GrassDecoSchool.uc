...
simulated function BeginPlay()
{
	...
	if( Level.NetMode==NM_DedicatedServer || NumGrass==0 )
		return;
	
	if( Skin )
		Variants.Add(Skin);
	else if( Variants.Size() )
		Skin = Variants[Rand(Variants.Size())];
	else return;
	...
	for( i=(NumGrass + 10); i>=0; --i )
	{
		...
		D.bUseLitSprite = bUseLitSprite;
		D.ActorRenderColor = ActorRenderColor;
		D.VisibilityRadius = VisibilityRadius;
		T = None;
		if( ++j==NumGrass )
			break;
	}
