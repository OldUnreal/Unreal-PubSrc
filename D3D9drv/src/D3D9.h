...
struct FGLVertexColor {
	...
	FGLVertexColor(FLOAT inX, FLOAT inY, FLOAT inZ, DWORD inColor)
		: x(inX), y(inY), z(inZ), color(inColor)
	{}
	FGLVertexColor(const FVector& inV, DWORD inColor)
		: x(inV.X), y(inV.Y), z(inV.Z), color(inColor)
	{}
	FGLVertexColor(const FVector& inV, FLOAT inA)
		: x(inV.X), y(inV.Y), z(inV.Z), color(0x00FFFFFF | (appFloor(inA * 255.f) << 24))
	{}
	FGLVertexColor(const FVector& inV)
		: x(inV.X), y(inV.Y), z(inV.Z), color(0xFFFFFFFF)
	{}
};
...
struct FGLSecondaryColor
{
	...
	FGLSecondaryColor(DWORD inSpecular)
		: specular(inSpecular)
	{}
};
...
typedef void (FASTCALL* FuncBufferVerts)(UD3D9RenderDevice*, FTransTexture*, INT);
...
class UD3D9RenderDevice : public URenderDevice {
	...
	bool m_gpFogEnabled, m_WithDistanceFog;
...
static inline DWORD BGRA_MAKE(BYTE b, BYTE g, BYTE r, BYTE a) {
	...
	bool FASTCALL LoadVertexProgram(IDirect3DVertexShader9 **, const DWORD *, const char *);
	bool FASTCALL LoadFragmentProgram(IDirect3DPixelShader9 **, const DWORD *, const char *);
