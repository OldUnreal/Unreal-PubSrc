...
UTexture* UBeamMesh::GetTexture( INT Count, AActor* Owner ) const
{
	guardSlow(UBeamMesh::GetTexture);
	if( Count )
		return (Owner->MultiSkins[Count-1] ? Owner->MultiSkins[Count-1] : Textures(0));
	return (Owner->Texture ? Owner->Texture : Textures(0));
	unguardSlow;
}
void UBeamMesh::GetTextures(UTexture** TexArray, AActor* Owner) const
{
	guardSlow(UBeamMesh::GetTextures);
	const INT Count = GetNumTextures();

	TexArray[0] = (Owner->Texture ? Owner->Texture : Textures(0));
	for (INT i = 1; i < Count; ++i)
		TexArray[i] = (Owner->MultiSkins[i - 1] ? Owner->MultiSkins[i - 1] : Textures(0));
	unguardSlow;
}
